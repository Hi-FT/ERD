...
def vg2classes():
    ...
    anno_file = '/mnt/ft/peizhi/data/VG/annotations/scene_graphs_preprocess'
    path_vg_img = '/mnt/ft/peizhi/data/VG/images/'
    dataset = json.load(open(anno_file + '.json', 'r'))
    
    classes_list = []
    anno_list = []

    for img_ids in dataset:
        # 取img ID
        file_name = path_vg_img + str(img_ids['image_id']) + '.jpg'
        print(file_name)

        # 取每个image上的objects
        for anno_ids in img_ids['objects']:
            obj_label = anno_ids['names']

            # 保存category列表
            classes_list.append(obj_label)
    
    classes_tuple=tuple(set(classes_list))
